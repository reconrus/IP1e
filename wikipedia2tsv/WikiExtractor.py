...
# DELETED Python 2.7 compatibiity by Viacheslav Iastrebov 
from urllib.parse import quote
from html.entities import name2codepoint
from itertools import zip_longest
from types import SimpleNamespace

# ADDED by Viacheslav Iastrebov
import csv 
import nltk
...
options = SimpleNamespace(
    ...
    # EDITED by Viacheslav Iastrebov
...
# ADDED by Viacheslav Iastrebov to extract first  
# sentence from page (get_first_sentence method)
nltk.download('punkt')
sent_detector = nltk.data.load('tokenizers/punkt/english.pickle')

def add_abbreviations(abbreviations_path):
    for line in codecs.open(abbreviations_path,'r','utf-8'):
...
# filter_disambig_page_pattern = re.compile("{{disambig(uation)?(\|[^}]*)?}}|__DISAMBIG__") # TODO DELETE
...
# def get_url(uid): # TODO DELETE OR CHANGE USECASE
#     return "%s?curid=%s" % (options.urlbase, uid)

def get_url(title):
    return "%s/%s" % (options.urlbase, title.replace(" ", "_"))
...
# EDITED by Viacheslav Iastrebov to save only first sentence in csv file 
class Extractor(object):
    ...
    def __init__(self, id, revid, title, lines):
        ...
        :param mode: part of text to extract ('sentence' to extract first sentence only) 
        """
        self.id = id
        self.revid = revid
        self.title = title
    ...
    def get_first_sentence(self, text, title):
        ...
        if not text:
            return result
        temp = sent_detector.tokenize(text[0])
        if len(temp) > 0:
            result = temp[0].strip().split('\n')[0]
        else:
            result = str()
        if result.strip() == title:
            result = result + (temp[1].strip().split('\n')[0] if len(sent_detector.tokenize(text[0])) > 1 else "")
        return result
    ...
    def clean(self, text):
        ...
        # if not options.toHTML: # TODO DELETE
...
def reduce_process(opts, output_queue, spool_length,
    ...
    if out_file:
        # EDITED by Vyacheslav Yastrebov to store in one file
        # instead of separate folder with multiple files 
...
def main():
    ...
    groupO.add_argument("--json", action="store_true",
                        help="write output in json format instead of the default one") # TODO DELETE (or change code to save as json)


    groupP = parser.add_argument_group('Processing')
    groupP.add_argument("-ab", "--abbreviations", 
                        help="Path to txt file with abbreviations for nltk tokenizer\
                             (that is used to extract first sentence)")
    # groupP.add_argument("--html", action="store_true", # TODO delete
    ...
    groupP.add_argument("--filter_disambig_pages", action="store_true", default=options.filter_disambig_pages, 
                        help="Remove pages from output that contain disabmiguation markup (default=%(default)s)") # TODO filter disambig pages always
    ...
    # if args.html:
    #     options.keepLinks = True # TODO DELETE
    ...
    if output_path != '-': # and not os.path.isdir(output_path):
        try:
            ...
            # os.makedirs(output_path) # TODO DELETE
        except:
    ...
    if args.abbreviations:
        if not os.path.isfile(args.abbreviations):
            logging.error('Abbreviation file "%s" doesn\'t exist!\n' % args.abbreviations)
            return 
